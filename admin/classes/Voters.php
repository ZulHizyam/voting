...
class Voters
{
    ...
    public function UPDATE_VOTER($name, $course, $year, $stud_id, $voter_id) {
        ...
        if($stmt->execute()) {
            echo "<div class='alert alert-success'>Voter was updated successfully.<a href='http://localhost/latest/admin/add_voters.php'><span class='glyphicon glyphicon-backward'></span></a></div>";
        }
        $stmt->free_result();
        return $stmt;
    }
    ...
    public function DELETE_VOTER($voter_id) {
        ...
        if($stmt->execute()) {
            header("location: http://localhost/latest/admin/add_voters.php");
            exit();
        }
        $stmt->free_result();
        return $stmt;
