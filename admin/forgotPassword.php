<?php
    ...
    if (isset($_POST['email'])) {
        ...
        $sql = $conn->query("SELECT id FROM admin WHERE email='$email'");
        if ($sql->num_rows > 0) {

            $token = generateNewString();

	        $conn->query("UPDATE admin SET token='$token', 
	        ...
	        $mail->setFrom("zhizyam99@gmail.com", "Hai");
	        ...
	        $mail->Body = "
	            ...
	            <a href='
	            http://localhost/latest/admin/forgotPassword.php?email=$email&token=$token
	            '>http://localhost/latest/admin/forgotPassword.php?email=$email&token=$token</a><br><br>
